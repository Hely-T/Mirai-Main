...
const name = args.join(" ")
  ...
  var callback = () => api.sendMessage({body: `Hi ?`,attachment: fs.createReadStream(__dirname + "/src/1.mp4")}, event.threadID, () => fs.unlinkSync(__dirname + "/src/1.mp4")); 
      return request(encodeURI(`${videoObj.hdLink}`)).pipe(fs.createWriteStream(__dirname+'/src/1.mp4')).on('close',() => callback());
       };
